...
│   │       ├── pages/               # 라우트 페이지
│   │       │   ├── home/            # 메인 페이지
│   │       │   ├── match/           # 대전 페이지 (in-game, match-result)
│   │       │   ├── single-play/     # 싱글플레이 페이지
│   │       │   ├── problem-bank/    # 문제 은행 페이지
│   │       │   ├── leaderboard/     # 리더보드 페이지
│   │       │   └── my-page/         # 마이페이지
│   │       ├── shared/              # 공용 컴포넌트 및 유틸리티
│   │       └── lib/                 # API 클라이언트 및 소켓 통신
│   │           ├── api/             # REST API 호출
...
│   │   └── src/
│   │       ├── auth/                # 인증 모듈
│   │       │   ├── strategies/      # Passport 전략 (GitHub, JWT)
│   │       │   ├── guards/          # 인증 가드
...
│   │       │   └── constants/       # 매칭 상수 정의
│   │       ├── match/               # 매치 기록 관리
│   │       ├── game/                # 실시간 대전 로직 (WebSocket)
│   │       ├── quiz/                # 퀴즈 출제 및 채점
│   │       │   └── clova/           # Clova Studio API 연동
│   │       ├── single-play/         # 싱글플레이 모드
│   │       ├── problem-bank/        # 문제 은행
│   │       ├── tier/                # 티어 시스템
│   │       ├── leaderboard/         # 리더보드
...
│       ├── retriever.py             # 문서 검색 (벡터 유사도)
│       ├── hyde_generator.py        # HyDE 쿼리 생성
│       ├── question_generator.py    # 문제 및 모범답안 생성
│       ├── evaluator.py             # RAG 품질 평가 (RAGAS)
│       ├── category_loader.py       # 카테고리별 문서 로딩
│       ├── schemas.py               # Pydantic 스키마 정의
│       ├── token_calculator.py      # 토큰 사용량 계산
│       ├── generate_questions_pipeline.py  # 문제 생성 배치 파이프라인
│       ├── ETL_Pipeline.ipynb       # 데이터 ETL 노트북
│       └── prompts/                 # LLM 프롬프트 템플릿
│
├── .github/
│   └── workflows/                   # CI/CD Workflows
│       ├── ci-develop.yml           # develop 브랜치 CI
│       ├── ci-cd-main.yml           # main 브랜치 CI/CD (NCP 배포)
│       └── pr-assign-review-merge.yml  # PR 자동화
│
├── docs/                            # 프로젝트 문서
