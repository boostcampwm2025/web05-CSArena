 ...
 *
 * 실행:
 *   k6 run scripts/benchmarks/match-history/load-test.js
 */
...
export default function (data) {
  ...
  requestCount.add(1);
  errorRate.add(!check(res, { 'match-history 200': (r) => r.status === 200 }));
}

export function teardown(data) {
  console.log(`매치 히스토리 부하테스트 완료 (유저 ${data.tokens.length}명)`);
}
